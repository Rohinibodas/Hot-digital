...
public class OxoMobileLiveHelper {
	...
	public void CreateNewAccount(String dataSet) throws Exception {
		try {
			Thread.sleep(3000);
			Sync.waitElementClickable(30, By.xpath("//a[@class='action create']"));
			Common.findElement("xpath", "//a[@class='action create']").click();
			Thread.sleep(3000);
		...
		try {
			...
			/*
			 * if(errormessagetextSize<=0){ } else{
			 * 
			 * ExtenantReportUtils.
			 * addFailedLog("verifying sign up page with valid field data",
			 * "see the fields populated with the data",
			 * "User failed to proceed signUp form",
			 * Common.getscreenShotPathforReport("signupissue")); Assert.fail();
			 * }
			 */
	...
	public void validatingSearchProductInformation(String productName) throws Exception {
		...
		try {
			 ...
			 * ; int totalproductscount=Common.findElements("xpath",
			 * "//li[@class='item product product-item ']").size();
			 */
			expectedResult = "in search result should show product image, product title, price add wishlist.";
			int totalproductscount = Common.findElements("xpath", "//li[contains(@class,'item product product-item ')]")
					.size();
	...
	public void NavigationToggle() throws InterruptedException {
		 ...
		 * By.xpath("//a[@class='social-login']"));
		 * //Common.javascriptclickElement("xpath","//a[@class='social-login']")
		 * ; Common.findElement("xpath", "//a[@class='social-login']").click();
		 * }catch(Exception |Error e){ Sync.waitElementClickable("xpath",
		 * "//li[@class='navigation__item navigation__item--no-parent navigation__item--account']"
		...
		try {
			...
			e.printStackTrace();
			Sync.waitElementClickable("xpath", "//a[@class='nav-opener open']");
			Common.clickElement("xpath", "//a[@class='nav-opener open']");
		}
	...
	public void Beverage() throws Exception {  		
		 ...
		 // Sync.waitAndSwitchToFrame("xpath","//ul[@data-menu='menu-15748']//li[@class='navigation__inner-item	// navigation__inner-item--all navigation__inner-item--level1']");			
		 Sync.waitElementClickable("xpath","//ul[@data-menu='menu-15748']//li[@class='navigation__inner-item navigation__inner-item--all navigation__inner-item--level1']");			
		 Common.clickElement("xpath","//ul[@data-menu='menu-15748']//li[@class='navigation__inner-item navigation__inner-item--all navigation__inner-item--level1']");  			
		 // Common.mouseOverClick("xpath", "//a[contains(text(),'Shop All')]");  		
		 } catch (Exception | Error e) {			
		e.printStackTrace();			
		 ...
		 selectproduct("Conical Burr Coffee Grinder");  	
		 }
	...
	public void clickBaby_Toddler() throws Exception {
		...
		try {
			...
			// Sync.waitAndSwitchToFrame("xpath","//ul[@data-menu='menu-15784']//li[@class='navigation__inner-item
			// navigation__inner-item--all navigation__inner-item--level1']");
			Sync.waitElementClickable("xpath",
					"//ul[@data-menu='menu-15784']//li[@class='navigation__inner-item navigation__inner-item--all navigation__inner-item--level1']");
			Common.clickElement("xpath",
					"//ul[@data-menu='menu-15784']//li[@class='navigation__inner-item navigation__inner-item--all navigation__inner-item--level1']");

			// Common.mouseOverClick("xpath", "//a[contains(text(),'Shop
			// All')]");
	...
	public void Click_CreditCard() throws InterruptedException {
		Thread.sleep(6000);
		try {
			Sync.waitPageLoad();
			Common.actionsKeyPress(Keys.PAGE_DOWN);
	...
	public void loginOxo(String dataSet) throws Exception {
		 ...
		 * Thread.sleep(2000); }
		 * ExtenantReportUtils.addPassLog("verifying Sign in link"
		 * ,"lands on sign popup", "User lands on the sign popup",
		 * Common.getscreenShotPathforReport("signpop")); } catch(Exception
		 * |Error e) { ExtenantReportUtils.addFailedLog("verifying Sign in link"
		 * ,"lands on sign popup", "User failed lands on sign popup",
		 * Common.getscreenShotPathforReport("signpop")); Assert.fail();
		 * 
		 * }
		 */
		try {
	...
	public void creditCard_payment2(String dataSet) throws Exception {
		Thread.sleep(6000);
		try {
			 ...
			 * "//select[@id='c-ct']").size(); Common.switchToDefault();
	...
	public void payPal_payment(String dataSet) {
		...
		try {
			...
			Thread.sleep(5000);
			Sync.waitElementClickable("xpath", "//div[contains(@class,'paypal-button-label-container')]");
			Common.javascriptclickElement("xpath", "//div[contains(@class,'paypal-button-label-container')]");
			// Common.clickElement("xpath",
			// "//div[contains(@class,'paypal-button-label-container')]");
			Common.switchToDefault();
			Thread.sleep(5000);
			...
			Common.textBoxInput("id", "email", data.get(dataSet).get("Email"));
			ExtenantReportUtils.addPassLog("verifying Paypal Payment page", "lands on Paypal payment window",
					"User lands on the Paypal payment window",
					Common.getscreenShotPathforReport("Paypalpaymentwindow"));

		}
		/*
		 * Common.textBoxInput("id", "password",
		 * data.get(dataSet).get("Password")); int
		 * sizeemail=Common.findElements("id", "email").size();
		 * 
		 * Common.assertionCheckwithReport(sizeemail>0,
		 * "verifying the paypal payment ", expectedResult,
		 * "open paypal site window", "faild to open paypal account"); }
		 * Common.clickElement("id", "btnLogin"); Thread.sleep(5000);
		 * Common.actionsKeyPress(Keys.END); Thread.sleep(5000);
		 * 
		 * int ButtonSize=Common.findElements("id", "confirmButtonTop").size();
		 * 
		 * if(ButtonSize>0){ Common.clickElement("id", "confirmButtonTop");
		 * }else{ Common.clickElement("id", "payment-submit-btn"); }
		 * Thread.sleep(8000); Common.switchToFirstTab(); }
		 */
		catch (Exception | Error e) {
			e.printStackTrace();
	...
	public void VerifyaingConformationPage() throws Exception {
		...
		Common.assertionCheckwithReport(CreateAccount.equals("Create Account"), "verifying Creat account button",
				"displaying the Creat account button", "successfully displaying the Creat account button",
	...
	public void acceptPrivecy() {
		int sizeofbutton = Common.findElements("id", "truste-consent-button").size();
		if (sizeofbutton > 0) {
	...
	public void ConformationPopup() throws InterruptedException {
		Thread.sleep(3000);
		int name = Common.findElements("xpath", "//button[@id='oxoux-30--cta-button']").size();
		if (name > 0) {
			Thread.sleep(3000);
			Sync.waitElementClickable("xpath", "//button[@id='oxoux-30--cta-button']");
			Common.clickElement("xpath", "//button[@id='oxoux-30--cta-button']");
		}
	}
	...
	public void ForgotPassword(String dataset) throws Exception {
		...
		/*
		 * Thread.sleep(3000); try{ Sync.waitElementClickable(30,
		 * By.xpath("//a[@class='social-login']")); Common.findElement("xpath",
		 * "//a[@class='social-login']").click(); Sync.waitElementClickable(30,
		 * By.id("email")); if(Common.findElement("id", "email")==null) {
		 * Common.mouseOverClick("xpath", "//a[@class='social-login']");
		 * Thread.sleep(3000); }
		 * ExtenantReportUtils.addPassLog("verifying Sign in link"
		 * ,"lands on sign page", "User lands on the sign popup",
		 * Common.getscreenShotPathforReport("signpopf")); } catch(Exception
		 * |Error e) { e.printStackTrace();
		 * ExtenantReportUtils.addFailedLog("verifying Sign in link"
		 * ,"lands on sign popup", "User failed lands on sign popup",
		 * Common.getscreenShotPathforReport("signpopf")); Assert.fail();
		 * 
		 * }
		 */
		...
		try {
			...
			Thread.sleep(5000);
			/*
			 * String message=Common.getText("xpath",
			 * "//div[contains(@class,'message-success')]/div");
			 * System.out.println(message); int
			 * size=Common.findElements("xpath",
			 * "//div[contains(@class,'message-success')]/div").size(); //int
			 * size=Common.findElements("xpath",
			 * "//input[@id='email_address_forgot']").size(); //
			 * Common.assertionCheckwithReport(size>0,
			 * "verifying forgetpassword option",
			 * expectedResult,"Successfully Forgot password pop up is displayed to customer"
	...
	public void HelpToggle() {
		...
		ExtenantReportUtils.addPassLog("Expanding Help Column toggle", "Help Column toggle should Expand",
				"Help Column toggle is Expanded", Common.getscreenShotPathforReport("HelpColumnToggle"));
	...
	public void Shipping_Returns() {
		...
		ExtenantReportUtils.addPassLog("Expanding Shipping & Returns toggle",
				"Shipping & Returns Column toggle should Expand", "Shipping & Returns Column toggle is Expanded",
				Common.getscreenShotPathforReport("ShippingReturnsColumnToggle"));
	...
	public void Company() {
		...
		ExtenantReportUtils.addPassLog("Expanding Company Column toggle", "Company Column toggle should Expand",
				"Company Column toggle is Expanded", Common.getscreenShotPathforReport("CompanyColumnToggle"));
		Common.clickElement("xpath", "//h4[contains(text(),'Company')]");
	}

	///////////////////////////////////////////////
	/*
	 * public void ContactUS() throws InterruptedException { HelpToggle();
	 * Thread.sleep(3000); // clickLogo(); try { //
	 ...
	 * ExtenantReportUtils.addPassLog("Validating ContactUS Link",
	 * "ContactUS link should be present", "ContactUS Link is present",
	 * Common.getscreenShotPathforReport("ContactUSLink"));
	 ...
	 * "//h2[contains(text(),'Email us')]"); String text = element.getText();
	 * 
	 ...
	 * Common.getscreenShotPathforReport("ContactUSText")); } else {
	 * 
	 * ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of ContactUS Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation ContactUS"); Assert.fail(); }
	 * 
	 * }
	 * 
	 * catch (Exception e) { e.printStackTrace(); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of ContactUS Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation ContactUS"); Assert.fail(); }
	 * 
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https", "ContactUS URL should Contains https",
	 * "ContactUS URL contains https", "ContactUS URL missing https"); //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https");
	 * Common.assertionCheckwithReport( Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/contact-us") &&
	 * Common.getPageTitle().equals("Contact Us"),
	 * "Validating Page Title and URL",
	 * "Actual and Current URL&Page Title Should be Same",
	 * "Actual and Current URL&Page Title are Same",
	 * "Actual and Current URL&Page Title are different"); }
	 * 
	 * public void ProductRegistration() throws InterruptedException {
	 * clickLogo(); Thread.sleep(3000); HelpToggle(); Thread.sleep(3000); try {
	 ...
	 * "ProductRegistration link should be able to click",
	 * "Clicked on ProductRegistration Link",
	 * Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14883']");
	 * Thread.sleep(3000); WebElement element = Common.findElement("xpath",
	 * "//h2[contains(text(),'Select your product')]"); String text =
	 * element.getText(); if (text.contains("Select your product")) {
	 * ExtenantReportUtils.
	 * addPassLog("Validating Webtext of Product Registration Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("ProductRegistrationText")); } else {
	 * ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of Product Registration Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation Product Registration"); Assert.fail(); } } catch
	 * (Exception e) { ExtenantReportUtils.
	 * addFailedLog("Validating  of Product Registration Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation Product Registration"); Assert.fail(); }
	 * 
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "Product Registration URL should Contains https",
	 * "Product Registration URL contains https",
	 * "Product Registration URL missing https"); //
	 * Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/productregistration")&&Common.
	 * getPageTitle().equals("Product // Registration"));
	 * Common.assertionCheckwithReport( Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/productregistration") &&
	 ...
	 * 
	 * 
	 * public void FAQ() throws InterruptedException { clickLogo();
	 * Thread.sleep(3000); HelpToggle(); Thread.sleep(3000); try {
	 * Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14882']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14882']"));
	 * ExtenantReportUtils.addPassLog("Validating FAQ Link",
	 * "FAQ link should be present", "FAQ Link is present",
	 * Common.getscreenShotPathforReport("FAQLink")); //
	 * ExtenantReportUtils.addPassLog("Validating FAQ Link","FAQ link // should
	 * be able to click" ,"Clicked on PFAQ //
	 * Link",Common.getscreenShotPathforReport("FAQ Link"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14882']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//span[contains(text(),'Frequently Asked Questions')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Frequently Asked Questions")) {
	 * System.out.println("Expected text is obtained");
	 * ExtenantReportUtils.addPassLog("Validating Webtext of FAQ Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("FAQText")); } else {
	 * System.out.println("Expected text is not obtained");
	 * ExtenantReportUtils.addFailedLog("Validating Webelement of FAQ Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation FAQ"); Assert.fail(); } } catch (Exception e) {
	 * ExtenantReportUtils.addFailedLog("Validating Webelement of FAQ Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation FAQ"); Assert.fail(); }
	 * 
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https", "FAQ URL should Contains https",
	 * "FAQ URL contains https", "FAQ URL missing https"); //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https");
	 * 
	 * // Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/faq")&&Common.getPageTitle().equals(
	 * "FAQ")); Common.assertionCheckwithReport(
	 * Common.getCurrentURL().equals("https://oxo-stg.heledigital.com/faq") &&
	 * Common.getPageTitle().equals("FAQ"), "Validating Page Title and URL",
	 * "Actual and Current URL&Page Title Should be Same",
	 * "Actual and Current URL&Page Title are Same",
	 * "Actual and Current URL&Page Title are different"); }
	 * 
	 * public void VoluntaryRecall() throws InterruptedException { clickLogo();
	 * Thread.sleep(3000); HelpToggle(); Thread.sleep(3000); try {
	 ...
	 * "VoluntaryRecall link should be present",
	 * "VoluntaryRecall Link is present",
	 * Common.getscreenShotPathforReport("VoluntaryRecallLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14884']"); //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https"); WebElement element
	 * = Common.findElement("xpath",
	 * "//span[contains(text(),'Tot Nest Booster Seat Voluntary Recall')]");
	 * String text = element.getText(); System.out.println("Text Obtained is" +
	 * text); if (text.contains("Tot Nest Booster Seat Voluntary Recall")) {
	 * System.out.println("Expected text is obtained"); ExtenantReportUtils.
	 * addPassLog("Validating Webtext of VoluntaryRecall Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("VoluntaryRecallText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of VoluntaryRecall Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation VoluntaryRecall"); Assert.fail(); }
	 * 
	 * }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of VoluntaryRecall Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation VoluntaryRecall"); Assert.fail(); }
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "VoluntaryRecall URL should Contains https",
	 * "VoluntaryRecall URL contains https",
	 * "VoluntaryRecall URL missing https"); Common.assertionCheckwithReport(
	 * Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/product-recall") &&
	 ...
	 * }
	 * 
	 * public void PrivacyPolicy() throws InterruptedException { clickLogo();
	 * Thread.sleep(3000); HelpToggle(); Thread.sleep(3000); try {
	 * Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14885']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14885']"));
	 * ExtenantReportUtils.addPassLog("Validating PrivacyPolicy Link",
	 * "PrivacyPolicy link should be present", "PrivacyPolicy Link is present",
	 * Common.getscreenShotPathforReport("PrivacyPolicyLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14885']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//span[contains(text(),'Privacy Policy')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is" + text); if
	 * (text.contains("Privacy Policy")) {
	 * System.out.println("Expected text is obtained");
	 * ExtenantReportUtils.addPassLog("Validating Webtext of PrivacyPolicy Page"
	 * , "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("PrivacyPolicyText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of PrivacyPolicy Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation PrivacyPolicy"); Assert.fail(); } }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of PrivacyPolicy Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "PrivacyPolicy URL should Contains https",
	 * "PrivacyPolicy URL contains https", "PrivacyPolicy URL missing https");
	 * //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https");
	 * 
	 * // Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/privacy")&&Common.getPageTitle().equals(
	 * "Privacy // Policy")); Common.assertionCheckwithReport(
	 ...
	 * ExtenantReportUtils.addPassLog("TermsandConditions Link",
	 * "TermsandConditions link should be present",
	 * "TermsandConditions Link is present",
	 * Common.getscreenShotPathforReport("TermsandConditionsLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14886']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//span[contains(text(),'Terms & Conditions')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is" + text); if
	 * (text.contains("Terms & Conditions")) {
	 * System.out.println("Expected text is obtained"); ExtenantReportUtils.
	 * addPassLog("Validating Webtext of TermsandConditions Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("TermsandConditionsText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of TermsandConditions Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation TermsandConditions"); Assert.fail(); }
	 * 
	 * }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "TermsandConditions URL should Contains https",
	 * "TermsandConditions URL contains https",
	 ...
	 * 
	 * // Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/Terms")&&Common.getPageTitle().equals("
	 * Terms // and Conditions")); Common.assertionCheckwithReport(
	 ...
	 * }
	 * 
	 * public void TrackOrder() throws InterruptedException { clickLogo();
	 * Thread.sleep(3000); Shipping_Returns(); Thread.sleep(3000); try {
	 * Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14888']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14888']"));
	 * ExtenantReportUtils.addPassLog("TrackOrder Link",
	 * "TrackOrder link should be present", "TrackOrder Link is present",
	 * Common.getscreenShotPathforReport("TrackOrderLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14888']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//span[contains(text(),'Track Order')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Track Order")) {
	 * System.out.println("Expected text is obtained");
	 * ExtenantReportUtils.addPassLog("Validating Webtext of TrackOrder Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("TrackOrderText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of TrackOrder Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation TrackOrder"); Assert.fail(); }
	 * 
	 * }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 ...
	 * "LinkValidation TrackOrder"); Assert.fail(); }
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https", "TrackOrder URL should Contains https",
	 * "TrackOrder URL contains https", "TrackOrder URL missing https"); //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https");
	 * System.out.println(Common.getPageTitle());
	 ...
	 * Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/sales/guest/form/")&&Common.getPageTitle
	 * ().equals("Orders // and Returns")); Common.assertionCheckwithReport(
	 * Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/sales/guest/form/") &&
	 ...
	 * clickLogo(); Thread.sleep(3000); Shipping_Returns(); Thread.sleep(3000);
	 * try { Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14889']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14889']"));
	 * ExtenantReportUtils.addPassLog("ShippingInformation Link",
	 * "ShippingInformation link should be present",
	 ...
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14889']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//h2[contains(text(),'Shipping Policies')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Shipping Policies")) {
	 * System.out.println("Expected text is obtained"); ExtenantReportUtils.
	 * addPassLog("Validating Webtext of ShippingInformation Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("ShippingInformationText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of ShippingInformation Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation ShippingInformation"); Assert.fail(); } }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of ShippingInformation Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "ShippingInformation URL should Contains https",
	 * "ShippingInformation URL contains https",
	 ...
	 * System.out.println(Common.getCurrentURL()); //
	 * Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/shipping")&&Common.getPageTitle().equals
	 * ("Shipping")); Common.assertionCheckwithReport(
	 ...
	 * }
	 * 
	 * public void BetterGuarantee() throws InterruptedException { clickLogo();
	 * Thread.sleep(3000); Shipping_Returns(); Thread.sleep(3000); try {
	 * Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14890']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14890']"));
	 * ExtenantReportUtils.addPassLog("BetterGuarantee Link",
	 * "BetterGuarantee link should be present",
	 * "BetterGuarantee Link is present",
	 * Common.getscreenShotPathforReport("BetterGuaranteeLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14890']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//span[contains(text(),'The OXO Better Guarantee*')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("The OXO Better Guarantee*")) {
	 * System.out.println("Expected text is obtained"); ExtenantReportUtils.
	 * addPassLog("Validating Webtext of BetterGuarantee Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("BetterGuaranteeText")); } else {
	 * System.out.println("Expected text is not obtained");
	 * 
	 * ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of BetterGuarantee Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation BetterGuarantee"); Assert.fail(); }
	 * 
	 * }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "BetterGuarantee URL should Contains https",
	 * "BetterGuarantee URL contains https",
	 ...
	 * System.out.println(Common.getCurrentURL()); //
	 * Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/guarantee")&&Common.getPageTitle().
	 * equals("The // OXO Better Guarantee")); Common.assertionCheckwithReport(
	 * Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/guarantee") &&
	 ...
	 * 
	 * clickLogo(); Thread.sleep(3000); Company(); Thread.sleep(3000); try {
	 * Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14892']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14892']"));
	 * ExtenantReportUtils.addPassLog("GoodTipsBlog Link",
	 * "GoodTipsBlog link should be present", "GoodTipsBlog Link is present",
	 * Common.getscreenShotPathforReport("GoodTipsBlogLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 ...
	 * WebElement element = Common.findElement("xpath",
	 * "//h1[contains(text(),'Good Tips')]"); String text = element.getText();
	 * System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Good Tips")) {
	 * System.out.println("Expected text is obtained");
	 * ExtenantReportUtils.addPassLog("Validating Webtext of GoodTipsBlog Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("GoodTipsBlogText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of GoodTipsBlog Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "GoodTipsBlog Product Registration"); Assert.fail(); }
	 * 
	 * }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "GoodTipsBlog URL should Contains https",
	 * "GoodTipsBlog URL contains https", "GoodTipsBlog URL missing https"); //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https");
	 * System.out.println(Common.getPageTitle());
	 * System.out.println(Common.getCurrentURL()); //
	 * Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/blog/")&&Common.getPageTitle().equals("
	 * OXO // Good Tips - The OXO Blog")); Common.assertionCheckwithReport(
	 ...
	 * ExtenantReportUtils.addPassLog("InventorSubmissions Link",
	 * "InventorSubmissions link should be present",
	 ...
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14893']"); WebElement
	 * element = Common.findElement("xpath",
	 * "//span[contains(text(),'Inventor Submission')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Inventor Submission")) {
	 * System.out.println("Expected text is obtained"); ExtenantReportUtils.
	 * addPassLog("Validating Webtext of InventorSubmissions Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("InventorSubmissionsText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of InventorSubmissions Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation InventorSubmissions"); Assert.fail(); } }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of InventorSubmissions Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "InventorSubmissions URL should Contains https",
	 * "InventorSubmissions URL contains https",
	 ...
	 * System.out.println(Common.getCurrentURL()); //
	 * Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://oxo-stg.heledigital.com/inventor")&&Common.getPageTitle().equals
	 * ("Inventor // Submission")); Common.assertionCheckwithReport(
	 ...
	 * }
	 * 
	 * public void Careers() throws InterruptedException { clickLogo();
	 * Thread.sleep(3000); Company(); Thread.sleep(3000); try {
	 * Sync.scrollDownToView("xpath", "//a[@data-menu='menu-14894']");
	 * Sync.waitElementClickable(10, By.xpath("//a[@data-menu='menu-14894']"));
	 * ExtenantReportUtils.addPassLog("Careers Link",
	 * "Careers link should be present", "Careers Link is present",
	 ...
	 * Thread.sleep(2000); Common.switchWindows();
	 * Sync.waitElementPresent("xpath",
	 * "//span[contains(text(),'Search for Jobs')]"); WebElement element =
	 * Common.findElement("xpath",
	 * "//span[contains(text(),'Search for Jobs')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Search for Jobs")) {
	 * System.out.println("Expected text is obtained");
	 * ExtenantReportUtils.addPassLog("Validating Webtext of Careers Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("CareersText")); } else {
	 * System.out.println("Expected text is not obtained");
	 * ExtenantReportUtils.addFailedLog("Validating Webelement of Careers Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation Careers"); Common.switchToFirstTab(); Assert.fail(); } }
	 * 
	 * catch (Exception e) {
	 * ExtenantReportUtils.addFailedLog("Validating Webelement of Careers Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 ...
	 * Thread.sleep(2000);
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https", "Careers URL should Contains https",
	 * "Careers URL contains https", "Careers URL missing https"); //
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * // "Validating URL which contains https", "This URL Contains https", //
	 * "give url contains https", "give url missing https");
	 * 
	 * Thread.sleep(6000); // Assert.assertTrue(Common.getCurrentURL().equals(
	 * "https://helenoftroy.wd1.myworkdayjobs.com/Main_HoT")&&Common.
	 * getPageTitle().equals("Search // for Jobs"));
	 * Common.assertionCheckwithReport( Common.getCurrentURL().equals(
	 * "https://helenoftroy.wd1.myworkdayjobs.com/Main_HoT") &&
	 ...
	 * ExtenantReportUtils.addPassLog("InvestorRelations Link",
	 * "InvestorRelations link should be present",
	 * "InvestorRelations Link is present",
	 * Common.getscreenShotPathforReport("InvestorRelationsLink")); //
	 * ExtenantReportUtils.addPassLog("Validating ProductRegistration //
	 * Link","ProductRegistration link should be able to click" // ,"Clicked on
	 * ProductRegistration //
	 * Link",Common.getscreenShotPathforReport("ProductRegistrationLink"));
	 * Common.clickElement("xpath", "//a[@data-menu='menu-14895']");
	 * Thread.sleep(2000); Common.switchWindows(); // Thread.sleep(4000);
	 * Sync.waitElementPresent("xpath",
	 * "//span[contains(text(),'Why Helen of Troy')]"); WebElement element =
	 * Common.findElement("xpath",
	 * "//span[contains(text(),'Why Helen of Troy')]"); String text =
	 * element.getText(); System.out.println("Text Obtained is..." + text); if
	 * (text.contains("Why Helen of Troy")) {
	 * System.out.println("Expected text is obtained"); ExtenantReportUtils.
	 * addPassLog("Validating Webtext of InvestorRelations Page",
	 * "Expected text should be obtained", "Expected text is obtained",
	 * Common.getscreenShotPathforReport("InvestorRelationsText")); } else {
	 * System.out.println("Expected text is not obtained"); ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of InvestorRelations Page",
	 * "Expected text should not be obtained", "Expected text is not obtained",
	 * "LinkValidation InvestorRelations"); Common.switchToFirstTab();
	 * Assert.fail(); } }
	 * 
	 * catch (Exception e) { ExtenantReportUtils.
	 * addFailedLog("Validating Webelement of InvestorRelations Page",
	 ...
	 * Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"),
	 * "Validating URL contains https",
	 * "InvestorRelations URL should Contains https",
	 * "InvestorRelations URL contains https",
	 * "InvestorRelations URL missing https"); Thread.sleep(3000);
	 * Common.assertionCheckwithReport( Common.getCurrentURL().equals(
	 * "https://investor.helenoftroy.com/overview/default.aspx") &&
	...
	public void ProductRegistration() throws InterruptedException {
		...
		Common.assertionCheckwithReport(Common.getCurrentURL().contains("https"), "Validating URL contains https",
				...
				"Product Registration URL missing https");
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/productregistration")&&Common.getPageTitle().equals("Product
		// Registration"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/product-registration")
						&& Common.getPageTitle().equals("OXO | Product Registration"),
	...
	public void ContactUS() throws InterruptedException {
		...
		// "give url contains https", "give url missing https");
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/contact-us")
						&& Common.getPageTitle().equals("Contact Us"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	}
	...
	public void FAQ() throws InterruptedException {
		...
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/faq")&&Common.getPageTitle().equals("FAQ"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/faq") && Common.getPageTitle().equals("FAQ"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	}
	...
	public void VoluntaryRecall() throws InterruptedException {
		...
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/product-recall")
						&& Common.getPageTitle().equals("Product Recall"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void PrivacyPolicy() throws InterruptedException {
		...
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/privacy")&&Common.getPageTitle().equals("Privacy
		// Policy"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/privacy")
						&& Common.getPageTitle().equals("Privacy Policy"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void TermsandConditions() throws InterruptedException {
		...
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/Terms")&&Common.getPageTitle().equals("Terms
		// and Conditions"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/Terms")
						&& Common.getPageTitle().equals("Terms and Conditions"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void TrackOrder() throws InterruptedException {
		...
		System.out.println(Common.getPageTitle());
		System.out.println(Common.getCurrentURL());
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/sales/guest/form/")&&Common.getPageTitle().equals("Orders
		// and Returns"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/sales/guest/form/")
						&& Common.getPageTitle().equals("Orders and Returns"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void ShippingInformation() throws InterruptedException {
		...
		System.out.println(Common.getPageTitle());
		System.out.println(Common.getCurrentURL());
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/shipping")&&Common.getPageTitle().equals("Shipping"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/shipping")
						&& Common.getPageTitle().equals("Shipping"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void BetterGuarantee() throws InterruptedException {
		...
		System.out.println(Common.getPageTitle());
		System.out.println(Common.getCurrentURL());
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/guarantee")&&Common.getPageTitle().equals("The
		// OXO Better Guarantee"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/guarantee")
						&& Common.getPageTitle().equals("The OXO Better Guarantee"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void GoodTipsBlog() throws InterruptedException {
		...
		System.out.println(Common.getPageTitle());
		System.out.println(Common.getCurrentURL());
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/blog/")&&Common.getPageTitle().equals("OXO
		// Good Tips - The OXO Blog"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/blog/")
						&& Common.getPageTitle().equals("OXO Good Tips - The OXO Blog"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void InventorSubmissions() throws InterruptedException {
		...
		System.out.println(Common.getPageTitle());
		System.out.println(Common.getCurrentURL());
		// Assert.assertTrue(Common.getCurrentURL().equals("https://www.oxo.com/inventor")&&Common.getPageTitle().equals("Inventor
		// Submission"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://www.oxo.com/inventor")
						&& Common.getPageTitle().equals("Inventor Submission"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
	...
	public void Careers() throws InterruptedException {
		...
		try {
			...
			Thread.sleep(2000);
			Common.switchWindows();
			// Common.scrollIntoView("xpath","//li//a[contains(text(),'Jobs')]");
			Sync.waitElementPresent("xpath", "//li//a[contains(text(),'Jobs')]");
			WebElement element = Common.findElement("xpath", "//li//a[contains(text(),'Jobs')]");
			String text = element.getText();
			System.out.println("Text Obtained is..." + text);
		...
		// for Jobs"));
		Common.assertionCheckwithReport(
				Common.getCurrentURL().equals("https://careers.helenoftroy.com/en")
						&& Common.getPageTitle().equals("Careers | Helen Of Troy"),
				"Validating Page Title and URL", "Actual and Current URL&Page Title Should be Same",
				"Actual and Current URL&Page Title are Same", "Actual and Current URL&Page Title are different");
		Common.switchToFirstTab();
	}
	...
	public void pinterest() throws InterruptedException {
		...
		try {
			...
			Thread.sleep(3000);
			WebElement element = Common.findElement("xpath", "//div[contains(text(),'Follow')]");
			String text = element.getText();
			System.out.println("Text Obtained is" + text);
	...
	public void Twitter() throws InterruptedException {
		...
		// (@oxo) • Instagram photos and videos"));
		Common.assertionCheckwithReport(Common.getCurrentURL().equals("https://mobile.twitter.com/OXO"),
	...
	public void clickGoodTipsBlog() {
		...
		try {
			...
			try {
				...
				Sync.waitElementClickable("xpath", "//li[@class='active']//a[contains(text(),'Good Tips Blog')]");
				Common.clickElement("xpath", "//li[@class='active']//a[contains(text(),'Good Tips Blog')]");
			}
	...
	public void signButton_GoodTipsBlog() throws Exception {
		...
		try {
			Sync.waitElementClickable("xpath", "//a[@class='sign-in']");
			Common.clickElement("xpath", "//a[@class='sign-in']");
	...
	public void forgetpasswordGoodTipsBlock(String dataset) throws Exception {
		...
		try {
			...
			// displayed to customer", "User faield to get Forgot password
			// pop");
			ExtenantReportUtils.addPassLog("verifying forgetpassword option", expectedResult,
					"Successfully Forgot password pop up is displayed to customer", "fotgot Password");
	...
	public void OXOLogin() throws InterruptedException {
		try {
			Thread.sleep(4000);
			login();
		} catch (Exception | Error e) {
	...
	public void iPadAcceptandClose() throws InterruptedException {

		Thread.sleep(3000);
		int name = Common.findElements("xpath", "//span[contains(text(),'Accept and Close')]").size();
		if (name > 0) {
			Thread.sleep(3000);
			Sync.waitElementClickable("xpath", "//span[contains(text(),'Accept and Close')]");
			Common.clickElement("xpath", "//span[contains(text(),'Accept and Close')]");
		}
	}

	public void AcceptPrivacy() throws InterruptedException {
		Thread.sleep(3000);
		int name = Common.findElements("xpath", "//button[contains(text(),'AGREE & PROCEED')]").size();
		if (name > 0) {
			Thread.sleep(3000);
			Sync.waitElementClickable("xpath", "//button[contains(text(),'AGREE & PROCEED')]");
			Common.clickElement("xpath", "//button[contains(text(),'AGREE & PROCEED')]");
		}

	}

	public OxoMobileLiveHelper(String datafile) {
